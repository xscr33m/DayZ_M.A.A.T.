...
namespace DayZ_MAAT._Core._Engine
{
    public class UpdateCheck
    {
        ...
        public static async Task CheckForUpdates()
        {
            try
            {
                var github = new GitHubClient(new ProductHeaderValue("DayZ_M.A.A.T."));

                string owner = "xscr33m";
                string repo = "DayZ_M.A.A.T.";
                ...
                try
                {
                    ...
                    if (latestVersion != null && latestVersion > currentVersion)
                    {
                        ...
                        {
                            string downloadUrl = string.Format("https://github.com/{0}/{1}/releases/download/{2}/xscr33mLabs_DayZ_MAAT.zip", owner, repo, sortedReleases.FirstOrDefault()?.TagName);
